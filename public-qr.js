...
function publicHubHtml({ business, menus }) {
  const menuHtml = menus.length ? menus.map(menu => `
    <section class="menu">
      <h2>${esc(menu.name)}</h2>
      ${(menu.items || []).length ? menu.items.map(item => `
        <div class="item">
          <div><strong>${esc(item.name)}</strong>${item.description ? `<p>${esc(item.description)}</p>` : ''}</div>
          ${item.price !== null && item.price !== undefined && item.price !== '' ? `<b>₹${esc(item.price)}</b>` : ''}
        </div>`).join('') : '<p class="muted">No items available yet.</p>'}
    </section>`).join('') : '<section class="empty">No published menus are available yet.</section>';

  return `<!doctype html><html lang="en"><head>
<meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<meta name="theme-color" content="#5b4bdb"><title>${esc(business.name)} — Customer Hub</title>
<style>
...
<div class="actions"><a href="#menus">🍽️ View Menu</a><button onclick="openFeedback()">⭐ Give Feedback</button></div>
<main id="menus">${menuHtml}</main><div class="footer">Powered by repute-tech.in</div></div>
<div class="modal" id="feedback" onclick="if(event.target===this)closeFeedback()"><div class="sheet"><h2>How was your experience?</h2><select id="rating"><option value="5">★★★★★ Excellent</option><option value="4">★★★★ Very good</option><option value="3">★★★ Good</option><option value="2">★★ Needs improvement</option><option value="1">★ Poor</option></select><textarea id="message" rows="4" placeholder="Tell us about your experience (optional)"></textarea><button onclick="submitFeedback()">Submit Feedback</button><button class="close" onclick="closeFeedback()">Close</button><p id="status" class="muted"></p></div></div>
<script>
const businessId=${JSON.stringify(business.id)};
function openFeedback(){document.getElementById('feedback').classList.add('open')}function closeFeedback(){document.getElementById('feedback').classList.remove('open')}
async function submitFeedback(){const s=document.getElementById('status');s.textContent='Submitting...';try{const r=await fetch('/api/customer/feedback',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({businessId,rating:Number(document.getElementById('rating').value),message:document.getElementById('message').value})});const d=await r.json().catch(()=>({}));if(!r.ok)throw new Error(d.error||'Unable to submit feedback');s.textContent='Thank you for your feedback!';}catch(e){s.textContent=e.message||'Unable to submit feedback';}}
</script></body></html>`;
}
...
async function servePublicQr(req, res, next) {
  try {
    ...
    const qr = await prisma.smartQr.findFirst({
      where: { slug },
      include: { business: { include: { menus: { where: { isPublished: true }, orderBy: { createdAt: 'desc' }, include: { items: { orderBy: { name: 'asc' } } } } } } }
    });
...
// server (1).js creates the real Express app internally and starts listening before
// server.js can access it. Register the public routes immediately before listen.
const originalListen = express.application.listen;
if (!express.application.__publicQrListenPatched) {
  express.application.__publicQrListenPatched = true;
  express.application.listen = function patchedListen(...args) {
    if (!this.__publicQrRegistered) {
      registerPublicQr(this);
      this.__publicQrRegistered = true;
    }
