...
</style></head><body><div class="wrap"><div class="brand">reputetechs.in</div><h1>Select your business type</h1><p>Choose the business type that best matches your business. reputetechs.in will use this selection to tailor your workspace and tools.</p><div class="card"><h2 style="font-size:18px;margin:0 0 5px">Choose your plan</h2><p style="margin:0 0 12px">Start with a <strong>7-day free trial</strong>. No payment is required during the trial.</p><div id="planGrid" class="grid" style="grid-template-columns:repeat(3,1fr)"><button class="category-card selected" data-plan="STARTER" onclick="selectPlan(this)"><strong>Starter</strong><b>₹499 / month</b><small>7-day free trial</small></button><button class="category-card" data-plan="GROWTH" onclick="selectPlan(this)"><strong>Growth Pro</strong><b>₹999 / month</b><small>7-day free trial</small></button><button class="category-card" data-plan="PRO" onclick="selectPlan(this)"><strong>Pro Plus</strong><b>₹1,499 / month</b><small>7-day free trial</small></button><button class="category-card" data-plan="STARTER_YEARLY" onclick="selectPlan(this)"><strong>Starter Yearly</strong><b>₹4,999 / year</b><small>7-day free trial</small></button><button class="category-card" data-plan="GROWTH_PRO_YEARLY" onclick="selectPlan(this)"><strong>Growth Pro Yearly</strong><b>₹9,999 / year</b><small>7-day free trial</small></button><button class="category-card" data-plan="PRO_YEARLY" onclick="selectPlan(this)"><strong>Pro Yearly</strong><b>₹12,999 / year</b><small>7-day free trial</small></button></div></div><div class="card"><div class="grid">${cards}</div><div id="otherField" class="field hidden"><label>Enter your business type</label><input id="customCategory" placeholder="e.g. Travel agency"></div><div class="field"><label>Business name</label><input id="businessName" minlength="2" maxlength="100" placeholder="Enter your business name"></div><div class="field"><label>Number of business owners</label><input id="ownerCount" type="number" min="1" max="20" step="1" value="1" placeholder="e.g. 1"></div><div class="field"><label>Business address</label><textarea id="businessAddress" maxlength="500" rows="3" placeholder="Enter your complete business address"></textarea></div><button class="btn" id="createBtn">Create business & continue</button><div id="msg" class="msg"></div></div></div><script>
...
function install(app){
  ...
  originalPost.call(app,'/api/businesses/create',async(req,res,next)=>{try{const user=await currentUser(req);if(!user)return res.status(401).json({error:'Authentication required'});const body=await readJsonBody(req);const name=String(body?.name||'').trim();const category=String(body?.category||'');const custom=String(body?.customCategory||'').trim();const ownerCount=Number.parseInt(body?.ownerCount,10);const address=String(body?.address||'').trim();const planCode=String(body?.planCode||'STARTER');if(name.length<2||name.length>100)return res.status(400).json({error:'Business name must be between 2 and 100 characters'});if(!Number.isInteger(ownerCount)||ownerCount<1||ownerCount>20)return res.status(400).json({error:'Number of business owners must be between 1 and 20'});if(address.length<3||address.length>500)return res.status(400).json({error:'Business address must be between 3 and 500 characters'});if(!CATEGORIES.some(x=>x.id===category))return res.status(400).json({error:'Invalid business type'});if(category==='other_businesses'&&!custom)return res.status(400).json({error:'Please enter your business type'});await ensureCategoryTable();await ensureBusinessDetailsTable();await ensureTrialColumns();const plan=await prisma.planCatalog.findUnique({where:{code:planCode}});if(!plan||!plan.active)return res.status(400).json({error:'Please select an available subscription plan'});const business=await prisma.business.create({data:{name,type:mappedPrismaType(category),slug:slugify(name),members:{create:{userId:user.id,role:'OWNER'}}}});await prisma.$executeRawUnsafe(`INSERT INTO "BusinessCategory" ("business_id","category","custom_category") VALUES ($1,$2,$3)`,business.id,category,category==='other_businesses'?custom:null);await prisma.$executeRawUnsafe(`INSERT INTO "BusinessDetails" ("business_id","owner_count","address") VALUES ($1,$2,$3)`,business.id,ownerCount,address);const trialStart=new Date();const trialEnd=new Date(trialStart.getTime()+7*24*60*60*1000);await prisma.subscription.create({data:{businessId:business.id,plan:plan.code,status:'TRIAL',monthlyPrice:plan.price,billingInterval:plan.billingInterval,provider:'trial',trialStartedAt:trialStart,trialEndsAt:trialEnd}});res.setHeader('Set-Cookie',`rp_business_id=${encodeURIComponent(business.id)}; Path=/; HttpOnly; SameSite=Lax; Max-Age=1209600`);res.status(201).json({ok:true,businessId:business.id,business})}catch(e){next(e)}});
