...
async function ensureBusinessDetailsTable(){
...
function setupPage(){
  ...
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Business Setup · reputetechs.in</title><link rel="preconnect" href="https://fonts.googleapis.com"><link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800&family=Material+Symbols+Outlined:opsz,wght,FILL,GRAD@20..48,400,0,0" rel="stylesheet"><style>
:root{--bg:#f8f9fa;--card:#fff;--ink:#191c1d;--muted:#666b75;--line:#e5e7eb;--p:#4f46e5;--p2:#7c3aed}*{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--ink);font-family:Inter,system-ui,sans-serif}.wrap{max-width:760px;margin:auto;padding:28px 18px 44px}.brand{font-weight:800;font-size:20px;margin-bottom:30px}.brand span{color:var(--p)}h1{font-size:30px;line-height:1.15;margin:0 0 8px}p{color:var(--muted);font-size:14px;line-height:1.6}.card{background:var(--card);border:1px solid var(--line);border-radius:18px;padding:18px;margin-top:18px}.grid{display:grid;grid-template-columns:repeat(2,1fr);gap:12px}.category-card{position:relative;text-align:left;min-height:145px;padding:15px;border:1px solid var(--line);border-radius:16px;background:#fff;cursor:pointer;display:flex;flex-direction:column;gap:7px;transition:.15s}.category-card.selected{border:2px solid var(--p);box-shadow:0 5px 18px rgba(79,70,229,.12)}.icon{font-size:25px;color:var(--p);background:#eef2ff;border-radius:11px;padding:8px;width:43px}.category-card strong{font-size:14px}.category-card small{font-size:11.5px;color:var(--muted);line-height:1.35}.check{position:absolute;right:12px;top:12px;width:21px;height:21px;border-radius:50%;border:2px solid #c7cbd3;color:transparent;text-align:center;font-size:12px;line-height:18px}.selected .check{background:var(--p);border-color:var(--p);color:#fff}.field{display:grid;gap:7px;margin-top:14px}.field label{font-size:12px;font-weight:700}.field input{width:100%;padding:12px;border:1px solid var(--line);border-radius:10px;font:inherit}.btn{border:0;border-radius:11px;background:linear-gradient(90deg,var(--p),var(--p2));color:#fff;padding:12px 16px;font-weight:700;cursor:pointer;width:100%;margin-top:14px}.msg{margin-top:10px;font-size:13px;color:#b42318}.hidden{display:none}@media(max-width:560px){.grid{grid-template-columns:1fr 1fr}.category-card{min-height:155px}h1{font-size:26px}}
</style></head><body><div class="wrap"><div class="brand">repute<span>techs.in</span></div><h1>Select your business type</h1><p>Choose the business type that best matches your business. reputetechs.in will use this selection to tailor your workspace and tools.</p><div class="card"><h2 style="font-size:18px;margin:0 0 5px">Choose your plan</h2><p style="margin:0 0 12px"><strong>7-day free trial on every plan.</strong> No payment is required during the trial.</p><div style="background:#fff7ed;border:1px solid #fed7aa;border-radius:10px;padding:10px;font-size:12px;color:#9a3412;line-height:1.5;margin-bottom:12px"><strong>Please select according to your business type and expected business volume.</strong> Selecting an unsuitable plan may require account review or correction before activation.</div><div id="planGrid" class="grid" style="grid-template-columns:repeat(3,1fr)"><button class="category-card selected" data-plan="STARTER" onclick="selectPlan(this)"><strong>Starter</strong><b>₹499 / month</b><small>Clothing, Salon, Photography, Gaming Café, Medical / Clinic · Smaller businesses</small></button><button class="category-card" data-plan="GROWTH" onclick="selectPlan(this)"><strong>Growth Pro</strong><b>₹999 / month</b><small>Restaurant / Hotel / Café, Automobile, Commercial Industries, Grocery, Gym / Fitness, Small Local Shops</small></button><button class="category-card" data-plan="PRO" onclick="selectPlan(this)"><strong>Pro Plus</strong><b>₹1,499 / month</b><small>Theater and other larger / complex businesses</small></button><button class="category-card" data-plan="STARTER_YEARLY" onclick="selectPlan(this)"><strong>Starter Yearly</strong><b>₹4,999 / year</b><small>Starter business types</small></button><button class="category-card" data-plan="GROWTH_PRO_YEARLY" onclick="selectPlan(this)"><strong>Growth Pro Yearly</strong><b>₹9,999 / year</b><small>Growth Pro business types</small></button><button class="category-card" data-plan="PRO_YEARLY" onclick="selectPlan(this)"><strong>Pro Yearly</strong><b>₹12,999 / year</b><small>Pro Plus business types</small></button></div></div><div class="card"><div class="grid">${cards}</div><div id="otherField" class="field hidden"><label>Enter your business type</label><input id="customCategory" placeholder="e.g. Travel agency"></div><div class="field"><label>Business name</label><input id="businessName" minlength="2" maxlength="100" placeholder="Enter your business name"></div><div class="field"><label>Business phone number</label><input id="businessPhone" type="tel" inputmode="tel" maxlength="20" placeholder="Enter your business phone number"></div><div class="field"><label>Business address</label><textarea id="businessAddress" maxlength="500" rows="3" placeholder="Enter your complete business address"></textarea></div><button class="btn" id="createBtn">Create business & continue</button><div id="msg" class="msg"></div></div></div><script>
let selected='restaurant_hotel_cafe';let selectedPlan='STARTER';const $=id=>document.getElementById(id);const esc=s=>String(s??'').replace(/[&<>\\"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','\\"':'&quot;',"'":'&#039;'}[c]));
function selectPlan(el){document.querySelectorAll('[data-plan]').forEach(x=>x.classList.remove('selected'));el.classList.add('selected');selectedPlan=el.dataset.plan}
function selectCategory(el){document.querySelectorAll('.category-card').forEach(x=>x.classList.remove('selected'));el.classList.add('selected');selected=el.dataset.id;$(\'otherField\').classList.toggle('hidden',selected!==\'other_businesses\');$(\'msg\').textContent=''}
...
$(\'businessName\').addEventListener('input',()=>$(\'msg\').textContent='');
$(\'createBtn\').onclick=async()=>{const name=$(\'businessName\').value.trim();$(\'msg\').textContent='';if(name.length<2){$(\'msg\').textContent='Please enter a valid business name.';return}if(name.length>100){$(\'msg\').textContent='Business name must be 100 characters or less.';return}if(selected==='other_businesses'&&!$(\'customCategory\').value.trim()){$(\'msg\').textContent='Please enter your business type.';return}try{$(\'createBtn\').disabled=true;$(\'createBtn\').textContent='Creating…';const businessPhone=$(\'businessPhone\').value.trim();const address=$(\'businessAddress\').value.trim();if(businessPhone.length<7||businessPhone.length>20){$(\'msg\').textContent='Please enter a valid business phone number.';return}if(address.length<3||address.length>500){$(\'msg\').textContent='Business address must be between 3 and 500 characters.';return}await req('/businesses/create',{method:'POST',body:JSON.stringify({name,category:selected,customCategory:$(\'customCategory\').value.trim(),planCode:selectedPlan,phone:businessPhone,address})});location.href='/'}catch(e){$(\'msg\').textContent=e.message;$(\'createBtn\').disabled=false;$(\'createBtn\').textContent='Create business & continue'}};
</script></body></html>`;
}
...
function install(app){
  ...
  originalPost.call(app,'/api/businesses/select',async(req,res,next)=>{try{const user=await currentUser(req);if(!user)return res.status(401).json({error:'Authentication required'});const body=await readJsonBody(req);const id=String(body?.businessId||'');const allowed=await prisma.business.findFirst({where:{id,...(['ADMIN','SUPER_ADMIN'].includes(user.role)?{}:{members:{some:{userId:user.id}}})}});if(!allowed)return res.status(404).json({error:'Business not found'});res.setHeader('Set-Cookie',`rp_business_id=${encodeURIComponent(id)}; Path=/; HttpOnly; SameSite=Lax; Max-Age=1209600`);res.json({ok:true,businessId:id})}catch(e){next(e)}});
  originalPost.call(app,'/api/businesses/create',async(req,res,next)=>{try{const user=await currentUser(req);if(!user)return res.status(401).json({error:'Authentication required'});const body=await readJsonBody(req);const name=String(body?.name||'').trim();const category=String(body?.category||'');const custom=String(body?.customCategory||'').trim();const phone=String(body?.phone||'').trim();const address=String(body?.address||'').trim();const planCode=String(body?.planCode||'STARTER');if(name.length<2||name.length>100)return res.status(400).json({error:'Business name must be between 2 and 100 characters'});if(phone.length<7||phone.length>20)return res.status(400).json({error:'Business phone number must be between 7 and 20 characters'});if(address.length<3||address.length>500)return res.status(400).json({error:'Business address must be between 3 and 500 characters'});if(!CATEGORIES.some(x=>x.id===category))return res.status(400).json({error:'Invalid business type'});if(category==='other_businesses'&&!custom)return res.status(400).json({error:'Please enter your business type'});await ensureCategoryTable();await ensureBusinessDetailsTable();await ensureTrialColumns();const plan=await prisma.planCatalog.findUnique({where:{code:planCode}});if(!plan||!plan.active)return res.status(400).json({error:'Please select an available subscription plan'});const business=await prisma.business.create({data:{name,type:mappedPrismaType(category),slug:slugify(name),phone,members:{create:{userId:user.id,role:'OWNER'}}}});await prisma.$executeRawUnsafe(`INSERT INTO "BusinessCategory" ("business_id","category","custom_category") VALUES ($1,$2,$3)`,business.id,category,category==='other_businesses'?custom:null);await prisma.$executeRawUnsafe(`INSERT INTO "BusinessDetails" ("business_id","owner_count","address") VALUES ($1,$2,$3)`,business.id,1,address);const trialStart=new Date();const trialEnd=new Date(trialStart.getTime()+7*24*60*60*1000);await prisma.subscription.create({data:{businessId:business.id,plan:plan.code,status:'TRIAL',monthlyPrice:plan.price,billingInterval:plan.billingInterval,provider:'trial',trialStartedAt:trialStart,trialEndsAt:trialEnd}});res.setHeader('Set-Cookie',`rp_business_id=${encodeURIComponent(business.id)}; Path=/; HttpOnly; SameSite=Lax; Max-Age=1209600`);res.status(201).json({ok:true,businessId:business.id,business})}catch(e){next(e)}});
