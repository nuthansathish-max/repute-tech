...
function setupPage(){
  ...
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Business Setup · repute-tech.in"><link rel="preconnect" href="https://fonts.googleapis.com"><link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800&family=Material+Symbols+Outlined:opsz,wght,FILL,GRAD@20..48,400,0,0" rel="stylesheet"><style>
...
function install(app){
  ...
  originalGet.call(app,'/api/businesses',async(req,res,next)=>{try{const user=await currentUser(req);if(!user)return res.status(401).json({error:'Authentication required'});const bs=await businessesFor(user);await ensureCategoryTable();const ids=bs.map(b=>b.id);let cats=[];if(ids.length)cats=await prisma.$queryRawUnsafe(`SELECT business_id, category, custom_category FROM "BusinessCategory" WHERE business_id IN (${ids.map(()=>'?').join(',')})`,...ids);const byId=new Map(cats.map(x=>[x.business_id,x]));const selected=getCookie(req,'rp_business_id');bs.sort((a,b)=>String(a.id)===String(selected)?-1:String(b.id)===String(selected)?1:0);res.json(bs.map(b=>{const c=byId.get(b.id);const label=c?.category==='other_businesses'?(c.custom_category||'Other Businesses'):CATEGORIES.find(x=>x.id===c?.category)?.label||b.type;return {...b,category:c?.category||null,customCategory:c?.custom_category||null,categoryLabel:label}}))}catch(e){next(e)}});
