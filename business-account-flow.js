...
function install(app){
  ...
  originalPost.call(app,'/api/businesses/create',async(req,res,next)=>{try{const user=await currentUser(req);if(!user)return res.status(401).json({error:'Authentication required'});const body=await readJsonBody(req);const name=String(body?.name||'').trim();const category=String(body?.category||'');const custom=String(body?.customCategory||'').trim();const planCode=String(body?.planCode||'STARTER');if(name.length<2||name.length>100)return res.status(400).json({error:'Business name must be between 2 and 100 characters'});if(!CATEGORIES.some(x=>x.id===category))return res.status(400).json({error:'Invalid business type'});if(category==='other_businesses'&&!custom)return res.status(400).json({error:'Please enter your business type'});await ensureCategoryTable();await ensureTrialColumns();const plan=await prisma.planCatalog.findUnique({where:{code:planCode}});if(!plan||!plan.active)return res.status(400).json({error:'Please select an available subscription plan'});const business=await prisma.business.create({data:{name,type:mappedPrismaType(category),slug:slugify(name),members:{create:{userId:user.id,role:'OWNER'}}}});await prisma.$executeRawUnsafe(`INSERT INTO "BusinessCategory" ("business_id","category","custom_category") VALUES ($1,$2,$3)`,business.id,category,category==='other_businesses'?custom:null);const trialStart=new Date();const trialEnd=new Date(trialStart.getTime()+7*24*60*60*1000);await prisma.subscription.create({data:{businessId:business.id,plan:plan.code,status:'TRIAL',monthlyPrice:plan.price,billingInterval:plan.billingInterval,provider:'trial',trialStartedAt:trialStart,trialEndsAt:trialEnd}});res.setHeader('Set-Cookie',`rp_business_id=${encodeURIComponent(business.id)}; Path=/; HttpOnly; SameSite=Lax; Max-Age=1209600`);res.status(201).json({ok:true,businessId:business.id,business})}catch(e){next(e)}});
