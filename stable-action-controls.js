(() => {
  ...
  async function reviewActions(){
    if(reviewRunning){reviewAgain=true;return}
    reviewRunning=true;
    try{
      ...
      reviewRunning=false;
      if(reviewAgain){reviewAgain=false;setTimeout(reviewActions,100)}
    }
  }
  ...
  async function decorateList(listId,endpoint,kind){
    if(decorateRunning.has(listId))return;
    const list=$(listId);if(!list)return;
    decorateRunning.add(listId);
    try{
      ...
      items.forEach((item,i)=>{
        const row=rows[i];
        if(!row||item.querySelector('[data-stable-edit]')||item.querySelector('[data-stable-delete]'))return;
        const a=document.createElement('div');a.className='row stable-actions';a.style.cssText='margin-top:10px;flex-wrap:wrap';
        const e=button('Edit');e.dataset.stableEdit=row.id;e.dataset.stableKind=kind;
        const d=button('Delete','danger');d.dataset.stableDelete=row.id;d.dataset.stableKind=kind;
        a.append(e,d);item.appendChild(a)
      });
    }catch{}finally{decorateRunning.delete(listId)}
  }
