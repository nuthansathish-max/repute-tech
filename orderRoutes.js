...
function registerOrderRoutes(app){
  ...
  originalGet.call(app,'/q/:slug/order',async(req,res,next)=>{
    try{
      ...
      const requestedMenuId=String(req.query.menuId||'').trim();
      const menuWhere={businessId:qr.businessId,isPublished:true,...(requestedMenuId?{id:requestedMenuId}:{})};
      const menu=await prisma.menu.findFirst({where:menuWhere,include:{items:true},orderBy:{createdAt:'desc'}});
      if(!menu)return res.status(404).send(requestedMenuId?'Selected menu not found or is not published':'No published menu available');
