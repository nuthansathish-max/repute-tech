...
function register(app){
  ...
  registered=true;
  app.get('/billing-pos/invoice/:orderId',async(req,res,next)=>{try{
    const u=await userFrom(req);if(!u)return res.status(401).send('Authentication required');
    const order=await prisma.order.findUnique({where:{id:req.params.orderId},include:{items:true,business:true}});
