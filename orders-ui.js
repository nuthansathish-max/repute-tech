(function(){
  ...
  async function loadOrders(){addNav();addSection();const list=$('ordersList');if(!list)return;list.innerHTML='<div class="sub">Loading orders…</div>';try{const context=await api('/business/status');window.reputeBusinessId=context.businessId;const orders=await api('/businesses/'+encodeURIComponent(context.businessId)+'/orders');const counts={PENDING:0,ACCEPTED:0,CANCELLED:0};orders.forEach(o=>counts[o.status]=(counts[o.status]||0)+1);updateBadge(counts.PENDING);$('orderSummary').innerHTML='<div class="card"><div class="label">Pending</div><div class="value">'+counts.PENDING+'</div></div><div class="card"><div class="label">Accepted</div><div class="value">'+counts.ACCEPTED+'</div></div><div class="card"><div class="label">Cancelled</div><div class="value">'+counts.CANCELLED+'</div></div>';if(!orders.length){list.innerHTML='<div class="sub">No orders yet. Orders from the published digital menu will appear here.</div>';return}list.innerHTML='';orders.forEach(o=>{const item=document.createElement('div');item.className='item';const head=document.createElement('div');head.innerHTML='<strong>'+esc(o.orderNumber)+'</strong> · <b>'+esc(o.status)+'</b><div class="sub" style="margin-top:6px">'+esc(o.customerName)+(o.customerPhone?' · '+esc(o.customerPhone):'')+'</div><div style="margin-top:10px">'+(o.items||[]).map(i=>esc(i.itemName)+' × '+Number(i.quantity||0)+' — '+money(i.lineTotal)).join('<br>')+'</div><div class="sub" style="margin-top:8px">'+new Date(o.createdAt).toLocaleString()+(o.notes?' · '+esc(o.notes):'')+'</div><div style="text-align:right;margin-top:8px"><b style="font-size:20px">'+money(o.total)+'</b><div class="sub">Pay at store</div></div>';item.appendChild(head);const actions=document.createElement('div');actions.style='display:flex;gap:8px;flex-wrap:wrap;margin-top:13px';['PENDING','ACCEPTED','CANCELLED'].forEach(st=>{const btn=document.createElement('button');btn.type='button';btn.className='btn '+(st==='ACCEPTED'?'':'secondary');btn.textContent=st;btn.disabled=o.status===st;btn.onclick=()=>updateOrderStatus(o.id,st);actions.appendChild(btn)});item.appendChild(actions);list.appendChild(item)})}catch(e){list.innerHTML='<div class="sub">'+esc(e.message)+'</div>'}}
  async function updateOrderStatus(id,status){if(status==='CANCELLED'&&!confirm('Cancel this order?'))return;try{await api('/orders/'+encodeURIComponent(id)+'/status',{method:'PATCH',body:JSON.stringify({status})});await loadOrders()}catch(e){notify(e.message)}}
