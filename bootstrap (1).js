...
import { publicHub } from './public-hub.js';
...
express.application.get=function(path,...handlers){
  ...
  if(path==='/api/businesses/:businessId/qr') return originalGet.call(this,path,async(req,res,next)=>{try{
  ...
  const result=await Promise.all(rows.map(async qr=>{
    const qrUrl=`${origin}/q/${encodeURIComponent(qr.slug)}`;
  ...
  // QR codes generated by older versions used /qr/:slug; keep that URL working while
  // all newly generated codes use the public /q/:slug customer hub route.
  if(path==='/q/:slug') return originalGet.call(this,path,async(req,res,next)=>{try{return await publicHub(req,res,req.params.slug)}catch(e){next(e)}});
...
express.application.post=function(path,...handlers){
  ...
  if(path==='/api/businesses/:businessId/qr') return originalPost.call(this,path,async(req,res,next)=>{
  try{
    const user=await sessionUser(req);
    ...
    if(!p.success){
      return res.status(400).json({
        error:p.error.issues[0]?.message || 'Invalid QR details'
      });
    }

    const existing=await prisma.smartQr.findFirst({
      where:{
        businessId,
        slug:p.data.slug
      }
    ...
    if(existing){
      return res.status(409).json({
        error:'That QR slug already exists'
      });
    }
    ...
    const qrUrl=
      `${origin}/q/${encodeURIComponent(p.data.slug)}`;

    const destination={
      type:'customer-hub'
    };

    const qr=await prisma.smartQr.create({
      data:{
        businessId,
        name:p.data.name,
        slug:p.data.slug,
