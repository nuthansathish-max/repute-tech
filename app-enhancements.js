(() => {
  ...
  async function enhanceQR(){
    ...
    async function render(){const b=await getBusiness();const rows=await req(`/businesses/${b.id}/qr`);list.innerHTML=rows.map(q=>{
      const url=`${location.origin}/qr/${encodeURIComponent(q.slug)}`;
      const img=`https://api.qrserver.com/v1/create-qr-code/?size=320x320&data=${encodeURIComponent(url)}`;
    ...
    btn.onclick=async()=>{btn.disabled=true;btn.textContent='Creating…';try{const b=await getBusiness();const name=$('qrName').value.trim();const slug=$('qrSlug').value.trim().toLowerCase();if(!name||!slug)throw new Error('Enter a QR name and slug');await post(`/businesses/${b.id}/qr`,{name,slug});notify('QR created successfully');await render();}catch(e){notify(e.message)}finally{btn.disabled=false;btn.textContent='Create QR'}};
  ...
  async function enhanceWhatsApp(){
    ...
    const left=page.querySelector('.layout > .card'); if(!left)return;
    const box=document.createElement('div');box.className='card';box.style.marginTop='14px';box.innerHTML=`<div class="section-title">WhatsApp delivery</div><div id="waStatus" class="sub">Checking connection…</div><div id="waCustomers" class="list" style="margin-top:10px;max-height:240px;overflow:auto"></div><div class="row" style="margin-top:10px"><button class="btn secondary" id="waPreview">Preview eligible</button><button class="btn" id="waSend">Send campaign</button></div><div id="waResult" class="reply" style="display:none"></div>`;page.appendChild(box);
  ...
  async function enhancePlans(){
    const grid=$('pricingGrid');if(!grid)return;try{const d=await req('/plans');if(!d.length)grid.innerHTML='<div class="notice">No active plans are configured yet. Ask the platform owner to seed the plan catalog.</div>';}catch(e){grid.innerHTML=`<div class="notice">Unable to load plans: ${esc(e.message)}</div>`}}
