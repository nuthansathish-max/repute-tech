...
import { getCookie, tokenHash, randomToken, verifyPassword } from './auth.js';
...
async function adminUser(req){
  ...
  if(!['ADMIN','SUPER_ADMIN'].includes(s.user.role))return null;
  return s.user;
}

function setAdminCookie(res,token){
  const parts=[`rp_admin_session=${token}`,'Max-Age=86400','Path=/','HttpOnly','SameSite=Lax'];
...
function install(app){
  ...
  originalPost.call(app,'/api/admin/login',async(req,res,next)=>{try{
    const email=String(req.body?.email||'').trim().toLowerCase();
    const password=String(req.body?.password||'');
    if(!email||!password)return res.status(400).json({error:'Enter admin email and password'});
    const user=await prisma.user.findUnique({where:{email}});
    if(!user||!['ADMIN','SUPER_ADMIN'].includes(user.role)||!user.passwordHash||!verifyPassword(password,user.passwordHash))return res.status(401).json({error:'Invalid admin email or password'});
    const token=randomToken();
    await prisma.session.create({data:{tokenHash:tokenHash(token),userId:user.id,expiresAt:new Date(Date.now()+86400000)}});
