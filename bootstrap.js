...
function slugify(value){
  return String(value||'business').toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-+|-+$/g,'').slice(0,50)||'business';
}
async function ensureBusiness(user){
  ...
  const slug = `${base}-${randomToken().slice(0,8)}`;
...
express.application.get = function(path,...handlers){
  if(path==='/auth/google'){
    return originalGet.call(this,path,async(_req,res,next)=>{
      try{
  ...
  if(path==='/auth/google/callback'){
    return originalGet.call(this,path,async(req,res,next)=>{
      try{
        ...
        const cookieState=getCookie(req,'rp_oauth_state');
        ...
        if(!email) return res.status(400).send('Google account email was not provided.');
        let user=await prisma.user.findUnique({where:{email}});
        if(!user){
          user=await prisma.user.create({data:{name:profile.name||email.split('@')[0],email,passwordHash:null,role:'OWNER'}});
        }
        const business=await ensureBusiness(user);
        await prisma.googleConnection.create({data:{userId:user.id,businessId:business.id,accessTokenEnc:encrypt(token.access_token),refreshTokenEnc:token.refresh_token?encrypt(token.refresh_token):undefined,expiresAt:token.expires_in?new Date(Date.now()+token.expires_in*1000):undefined,scope:token.scope,googleAccountId:null}});
        await startSession(user,res);
        res.setHeader('Set-Cookie',[
          `rp_session=${getCookie(req,'rp_session')||''}`,
          `rp_oauth_state=; Max-Age=0; Path=/; HttpOnly; SameSite=Lax${process.env.NODE_ENV==='production'?'; Secure':''}`
        ]);
...
express.application.post = function(path,...handlers){
  if(path==='/api/auth/signup'){
    return originalPost.call(this,path,async(req,res,next)=>{
      try{
        const schema=z.object({name:z.string().min(2).max(80),email:z.string().email(),password:z.string().min(8).max(200)});
