...
function sessionCookie(token){
  const parts=[`rp_session=${token}`,`Max-Age=${14*86400}`,'Path=/','HttpOnly','SameSite=Lax'];
  if(process.env.NODE_ENV==='production')parts.push('Secure');
  return parts.join('; ');
}
function clearOAuthCookie(){
  return `rp_oauth_state=; Max-Age=0; Path=/; HttpOnly; SameSite=Lax${process.env.NODE_ENV==='production'?'; Secure':''}`;
}
...
express.application.get = function(path,...handlers){
  ...
  if(path==='/auth/google/callback') return originalGet.call(this,path,async(req,res,next)=>{try{
    ...
    const sessionToken=await startSession(user);
    res.setHeader('Set-Cookie',[sessionCookie(sessionToken),clearOAuthCookie()]);
